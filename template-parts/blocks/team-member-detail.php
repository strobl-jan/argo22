...
$member = get_field('member'); // může být ID nebo WP_Post
$member_id = 0;
...
// Pokud není vybrán člen, ukaž placeholder
if (!$member_id) : ?>
...
// Poslední příspěvky podle Reviewera
$recent_posts = function_exists('teamth_get_recent_posts_by_reviewer')
    ? teamth_get_recent_posts_by_reviewer($member_id, 5)
    : [];

?>
<section class="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <div class="flex flex-col items-center gap-6 md:flex-row md:items-start">
        <div class="shrink-0">
            <?php if ($image_url): ?>
                <img src="<?php echo esc_url($image_url); ?>"
                     alt="<?php echo esc_attr($image_alt); ?>"
                     class="h-32 w-32 rounded-full object-cover ring-2 ring-slate-200 dark:ring-slate-700" />
            <?php else: ?>
                <div class="flex h-32 w-32 items-center justify-center rounded-full bg-slate-100 text-slate-400 dark:bg-slate-800">
        ...
        <div class="flex-1">
            <header class="mb-4">
                ...
                <?php if ($position): ?>
                    <p class="mt-1 text-slate-500 dark:text-slate-400"><?php echo esc_html($position); ?></p>
                <?php endif; ?>
            </header>
            ...
            <?php if (!empty($recent_posts)): ?>
                <div class="mt-8">
                    <h4 class="mb-3 text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                        Nejnovější články člena
                    </h4>
                    <ul class="list-inside list-disc space-y-1 text-slate-800 dark:text-slate-100">
                        <?php foreach ($recent_posts as $p): ?>
                            <li>
                                <a class="hover:underline" href="<?php echo esc_url(get_permalink($p)); ?>">
